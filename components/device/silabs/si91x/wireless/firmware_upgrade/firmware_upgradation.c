...
sl_status_t
    ...
    while(offset < fw_image_size) {
        ...
        retval =
            sli_si91x_boot_instruction((uint8_t)boot_insn, (uint16_t*)(firmware_image + offset));
...
#ifdef SLI_SI91X_OFFLOAD_NETWORK_STACK
sl_status_t sl_si91x_ota_firmware_upgradation(
    ...
    if(server_ip.type == SL_IPV4) {
        ...
        memcpy(
            otaf_fwup.server_ip_address.ipv4_address,
            server_ip.ip.v4.bytes,
            SL_IPV4_ADDRESS_LENGTH);
    } else if(server_ip.type == SL_IPV6) {
        otaf_fwup.ip_version = SL_IPV6;
        memcpy(
            otaf_fwup.server_ip_address.ipv6_address,
            server_ip.ip.v6.bytes,
...
sl_status_t sl_si91x_http_otaf(
    uint8_t type,
    uint16_t flags,
    uint8_t* ip_address,
    uint16_t port,
    ...
    //ssl versions
    https_enable = (flags & SL_SI91X_TLS_V_1_0) ? (https_enable | SL_SI91X_TLS_V_1_0) :
                                                  https_enable;

    https_enable = (flags & SL_SI91X_TLS_V_1_1) ? (https_enable | SL_SI91X_TLS_V_1_1) :
                                                  https_enable;

    https_enable = (flags & SL_SI91X_TLS_V_1_2) ? (https_enable | SL_SI91X_TLS_V_1_2) :
                                                  https_enable;

    https_enable = (flags & SL_SI91X_TLS_V_1_3) ? (https_enable | SL_SI91X_TLS_V_1_3) :
                                                  https_enable;

    // Set HTTP version 1.1 feature bit
    https_enable = (flags & SL_SI91X_HTTP_V_1_1) ? (https_enable | SL_SI91X_HTTP_V_1_1) :
                                                   https_enable;

    https_enable = (flags & SL_SI91X_HTTPS_CERTIFICATE_INDEX_1) ?
                       (https_enable | SL_SI91X_HTTPS_CERTIFICATE_INDEX_1) :
                       https_enable;

    https_enable = (flags & SL_SI91X_HTTPS_CERTIFICATE_INDEX_2) ?
    ...
    if(flags & SL_SI91X_HTTPS_USE_SNI) {
        ...
        sl_si91x_socket_type_length_value_t* set_sni =
            (sl_si91x_socket_type_length_value_t*)malloc(
    ...
    host_name = ((flags & SL_SI91X_HTTP_V_1_1) && (sl_strlen((char*)host_name) == 0)) ?
                    ip_address :
                    host_name;
    ...
    if(http_length > SLI_SI91X_HTTP_BUFFER_LEN ||
       sl_strnlen(((char*)resource), SLI_SI91X_MAX_HTTP_URL_SIZE + 1) >
           SLI_SI91X_MAX_HTTP_URL_SIZE)
    ...
    if(http_length <= SLI_SI91X_MAX_HTTP_CHUNK_SIZE) {
        ...
        while(rem_length) {
            if(rem_length > SLI_SI91X_MAX_HTTP_CHUNK_SIZE) {
                packet_identifier = (offset == 0) ? SLI_HTTP_GET_FIRST_PKT :
                                                    SLI_HTTP_GET_MIDDLE_PKT;
            ...
            status = sl_si91x_custom_driver_send_command(
                ...
                (rem_length == chunk_size) ? SLI_SI91X_WAIT_FOR_OTAF_RESPONSE :
